 ...
 *
 * Under jsdom, `Storage` is an *exotic* object whose named-property setter
 * swallows `vi.spyOn(localStorage, 'setItem')` — the underlying write still
 * succeeds, but the spy records **zero** calls, silently breaking any test
 ...
 * normally. These tests FAIL without that fix.
 */
