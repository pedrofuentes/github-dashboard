...
- **Delegation ledger (`PLAN.md`).** Per increment and per gate artifact, record the **producer** sub-agent id and the **reviewer** sub-agent id (+ PR/artifact ref). The invariant **producer ≠ reviewer** must hold at every entry; the watchdog audits it and flags a **Lead-solo collapse** — the Lead authored *and* reviewed, or no sub-agent was used — as a WARNING. Also externalize the orchestrator's **in-flight state** here (open increments, claims, pending rebases) so a fresh session or watchdog tick resumes idempotently.
