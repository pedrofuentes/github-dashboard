...
**Decision**: Restrict the **runtime network allowlist to GitHub-owned origins only**: `api.github.com`, `github.com/login/*` (future auth), and `*.githubusercontent.com` (avatars / raw images). Ship a **strict Content-Security-Policy** whose `connect-src` lists GitHub-owned origins only (`default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' https://*.githubusercontent.com; connect-src https://api.github.com; object-src 'none'`). **Bundle fonts/assets locally** — no CDN/analytics/third-party origins. Forbid `eval()`/`new Function()`; render API strings via `textContent`, never `innerHTML`. **Verify with an automated Playwright network-interception test** asserting **0** requests to non-GitHub origins (a Definition-of-Done item; `ROADMAP.md` M5).
...
**Decision**: Deploy to **GitHub Pages via GitHub Actions** (`build_type: workflow`). Set Vite **`base: '/github-dashboard/'`** so assets resolve under the project path, and add an **SPA fallback** by copying `index.html` → `404.html` so deep links resolve client-side. Pages is **already enabled** (DECISION issue #1) with a **custom domain `pedrofuent.es`**; the live URL is **`http://pedrofuent.es/github-dashboard/`** (HTTPS cert approved). The deploy workflow is pre-authorized to author; the production switch is the cofounder's (now flipped).
...
**Consequences**: Fully static, no server. The base path means all asset URLs are prefixed with `/github-dashboard/`; the `404.html` fallback enables client-side routing and deep links. The custom domain is provisioned with an approved HTTPS certificate (note: `https_enforced` is currently `false`). Go-live remains a human toggle, now satisfied.
